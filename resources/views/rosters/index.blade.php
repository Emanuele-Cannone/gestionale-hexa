        ...
        let calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventDidMount: function(arg) {
                ...
                if(selector){
                    ...
                    let val = selector.value.toLowerCase();
    
                    if (!(arg.event.extendedProps.user_name.toLowerCase().includes(val) || val == "all")) {
    ...
    <div>
        ...
        <x-slot name="header">
            ...
            <nav class="justify-between text-gray-700 sm:flex sm:px-5 bg-gray-50 dark:bg-gray-800" aria-label="Breadcrumb">
                ...
                @if((Auth::check() && Auth::user()->hasRole('Admin')) || Auth::user()->hasPermissionTo('import roster'))
                    <div>
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <form action="{{route('importRosterFile')}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="roster_help">{{ __('roster.roster_import_message') }}</div>
                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <select name="weekNumber" required>
                                @foreach ($weeksOfYearAvailable as $weekOfYearAvailable)
                                    <option value="{{ $weekOfYearAvailable }}" {{ $weekOfYearAvailable == $currentWeekOfYear ? 'selected' : '' }} >{{ $weekOfYearAvailable }}</option>
                                @endforeach
                            </select>
                        </div>
                        <!-- Modal footer -->
